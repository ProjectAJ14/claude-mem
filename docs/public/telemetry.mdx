---
title: "Telemetry"
description: "Removed in this fork — no analytics are sent"
---

# Telemetry

<Warning>
**This fork sends no telemetry.** `explainTelemetryConsent()` in
`src/services/telemetry/consent.ts` returns disabled unconditionally, so no
PostHog client is ever constructed and no event is ever queued — not with
`CLAUDE_MEM_TELEMETRY=1`, not with a `telemetry.json` recording consent. There
is no installer consent prompt, and `npx claude-mem telemetry enable` refuses.
`npx claude-mem telemetry status` always reports DISABLED.

The instrumentation code below is left in the tree unreached, so upstream syncs
keep applying cleanly. The rest of this page describes what upstream collects,
and is kept only as a record of what the disabled path would have sent.
</Warning>
